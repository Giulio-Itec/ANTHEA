    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        if (e.Args.Contains("--check"))
        {
            window.Loaded += (_, _) =>
            {
                ...
                {
                    window.Check();
                    File.WriteAllText(Path.Combine(AppContext.BaseDirectory, "verifica.txt"), "OK: composizione ATECAP, esposizioni singole e combinate, aria in funzione del Dmax, input invalidi; proprietà, aderenza, copriferro NTC/EC2, tolleranze e tre colonne a 1500/1200 px.");
                    Shutdown(0);
                }
                catch (Exception ex)
                {

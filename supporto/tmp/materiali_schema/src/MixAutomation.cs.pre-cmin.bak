...
public sealed partial class MaterialWindow
{
    ...
    UIElement AutoNumeric(string key,string label,string unit)
    {
        ...
        void ModeChanged()
        {
            ...
            box.ToolTip=automatic.IsChecked==true?"Proposta automatica, modificabile. Non è un dosaggio qualificato né un dato misurato.":"Valore manuale: conservato al cambio delle altre scelte.";
            RefreshDetails();
        }
    ...
    void CheckAutomaticMix()
    {
        ...
        Equal("cementMass",260);Equal("totalWater",169);Blank("air");
        exposureChecks["XC4"].IsChecked=true;Equal("cementMass",300);Equal("totalWater",150);
        numbers["cementMass"].Text="360";Equal("totalWater",180);
        exposureChecks["XS3"].IsChecked=true;Equal("cementMass",360);Equal("totalWater",162);
        ...
        choices["cement"].SelectedIndex=1;Blank("cementMass");Blank("totalWater");Blank("air");
        ...
        exposureChecks["X0"].IsChecked=true;Blank("cementMass");Blank("totalWater");

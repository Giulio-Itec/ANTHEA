...
{
    readonly TabControl sections=new() { Background=Brushes.Transparent, BorderThickness=new Thickness(0) };
    ...
    readonly TextBlock coverHeadline=Text("",28,true),coverSteps=Text(""),exposureDetails=Text(""),mixResults=Text(""),mixLimits=Text(""),mixNotes=Text("",12),coverNotes=Text("",12),consistencyInfo=Text("",12);
    readonly CheckBox highStrength=new(){Content="Riduzione per classe di resistenza (tab. 4.3N)",Margin=new Thickness(0,6,0,6)},
        slab=new(){Content="Geometria a piastra; armature non influenzate dal getto",Margin=new Thickness(0,6,0,6)},
        quality=new(){Content="Controllo speciale della produzione del calcestruzzo",Margin=new Thickness(0,6,0,6)},
        rough=new(){Content="Superficie irregolare / aggregato a vista (+5 mm)",Margin=new Thickness(0,6,0,6)};
    bool detailsReady;
    ...
    UIElement Field(string label,FrameworkElement control,string unit="")
    {
        ...
        var t=Text(label); t.Margin=new Thickness(0,0,12,0); g.Children.Add(t);
        control.Margin=new Thickness(0,0,8,0); control.MinHeight=28; Grid.SetColumn(control,1); g.Children.Add(control);
    ...
    {
        sections.Items.Add(new TabItem{Header="Proprietà meccaniche",Content=Scroller(properties)});
        var checks=new WrapPanel();
        foreach(var e in Durability.Exposures)
        {
            var cb=new CheckBox {Content=e.Code,Width=76,Margin=new Thickness(0,7,0,7),ToolTip=e.Description,IsChecked=e.Code=="XC1"};
            exposureChecks.Add(e.Code,cb); checks.Children.Add(cb); cb.Checked+=(_,_)=>RefreshDetails(); cb.Unchecked+=(_,_)=>RefreshDetails();
        }
        foreach(var cb in new[]{highStrength,slab,quality,rough}) {cb.Checked+=(_,_)=>RefreshDetails();cb.Unchecked+=(_,_)=>RefreshDetails();}
        var exposure=Paper(Stack(Text("Classi di esposizione",18,true),Text("Seleziona le azioni contemporanee sulla stessa superficie. Passa sul codice per la descrizione.",12),checks,exposureDetails));
        var cover=Paper(Stack(Text("Dati per il copriferro",18,true),Text("Armatura ordinaria a barre isolate · calcestruzzo normale",12),
        ...
        var result=Paper(Stack(Text("Copriferro nominale",18,true),coverHeadline,coverSteps,coverNotes));
        var exposureBody=Stack(exposure,cover,result); exposureBody.Margin=new Thickness(0,16,0,0);
        sections.Items.Add(new TabItem {Header="Esposizione e copriferro",Content=Scroller(exposureBody)});
        var mix=Paper(Stack(Text("Composizione per 1 m³ di calcestruzzo",18,true),
        ...
        var fresh=Paper(Stack(Text("Calcestruzzo fresco e specificazione",18,true),
            ...
            Text("Dmax è condiviso con la scheda copriferro. Le classi di consistenza sono definite dal prospetto 3; per S1 e S5 considerare i limiti di sensibilità della prova (§5.4.1).",12)));
        var compositionBody=Stack(mix,fresh,Paper(Stack(Text("Valori limite raccomandati · F.1",18,true),mixLimits,mixResults,mixNotes))); compositionBody.Margin=new Thickness(0,16,0,0);
        sections.Items.Add(new TabItem{Header="Composizione",Content=Scroller(compositionBody)});
    ...
    void RefreshDetails()
    {
        ...
        try
        {
            ...
            if(active.Any(e=>e.CoverColumn<0)) coverSteps.Text+="\nXF/XA: requisiti aggiuntivi della miscela nella scheda Composizione.";
        }

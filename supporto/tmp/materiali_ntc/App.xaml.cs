    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        if (e.Args.Contains("--check"))
        {
            window.Loaded += (_, _) =>
            {
                ...
                {
                    window.Check();
                    File.WriteAllText(Path.Combine(AppContext.BaseDirectory, "verifica.txt"), "OK: proprietà centrate, aderenza, esposizioni, copriferro NTC/EC2, acqua efficace, validazione input e tre colonne a 1500/1200 px.");
                    Shutdown(0);
                }
                catch (Exception ex)
                {

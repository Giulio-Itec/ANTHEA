...
public partial class App : Application
{
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        MainWindow = window;
        if(e.Args.Contains("--check-display"))
        {
            window.ContentRendered+=(_,_)=>
            {
                try { File.WriteAllText(Path.Combine(AppContext.BaseDirectory,"verifica-schermo.txt"),DisplayAdaptation.CheckMaximized(window)); Shutdown(0); }
                catch(Exception ex) { File.WriteAllText(Path.Combine(AppContext.BaseDirectory,"verifica-schermo.txt"),ex.ToString()); Shutdown(1); }
            };
        }
        if (e.Args.Contains("--check"))
        {
            window.Loaded += (_, _) =>
            {
                ...
                {
                    window.Check();
                    File.WriteAllText(Path.Combine(AppContext.BaseDirectory, "verifica.txt"), "OK: composizione automatica, aggiornamento esposizioni, acqua manuale/cemento automatico, conservazione modifiche manuali, ripristino Auto, input invalidi e fuori campo; proprietà, aderenza, copriferro NTC/EC2, tolleranze e tre colonne a 1500/1200 px.");
                    Shutdown(0);
                }
                catch (Exception ex)
                {
...
public sealed partial class MaterialWindow : Window
{
    ...
    public void Check()
    {
        ...
        foreach(var automatic in mixAuto.Values)automatic.IsChecked=true;

...
if(args.Length==3&&args[0]=="--calcola")
{
    ...
    var result=module==BridgeSection.Module?BridgeSection.Calculate(data.AsObject()).Json():module is PaloOrizzontale.Module or MicropaloOrizzontale.Module?PaloOrizzontale.Calculate(data.AsObject()):module=="str_palo"?CalcoloSezione.Calcola(data.AsObject()):Calcolo.Calcola(data,module=="geo_micropalo_verticale");

...
{
    public static int Run(JsonArray cases, Action<string>? progress = null, string? outputDirectory = null)
    {
        int passed=0;
        void Assert(bool condition,string message){if(!condition)throw new Exception(message);passed++;}
        var root=Path.GetFullPath(outputDirectory ?? Path.Combine("supporto", "artefatti", "verifiche-software"));
        var temp=Path.Combine(root,"run-"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(temp);
        try
        {
            progress?.Invoke("Inizio verifiche software");
            ...
            {
                progress?.Invoke("Archivio: " + module);
                var d=module=="geo_palo_verticale"?data:module=="geo_micropalo_verticale"?cases.First(c=>c.S("tipo")=="micropalo")!["input"]!.AsObject():Archivio.NuovoFoglio(module);
            ...
            {
                progress?.Invoke("Report: " + (micro ? "micropalo" : "palo"));
                ...
                progress?.Invoke("Tabelle completate");
                ...
                progress?.Invoke("Report completo scritto");
                ...
                progress?.Invoke("XML verificato");
                ...
                progress?.Invoke("Report parziale verificato");
            }
            ...
            {
                progress?.Invoke("Esito Word: " + expected);
            ...
            progress?.Invoke("Calcolo sezione storico");
            ...
            progress?.Invoke($"Completato: {passed} controlli superati.");
            Console.WriteLine($"Controlli software, archivi e report: {passed} superati.");return passed;
        }
        finally
        {
            // Keep artifacts and the last completed assertion for diagnosing interrupted runs.
            File.WriteAllText(Path.Combine(temp,"controlli-eseguiti.txt"),passed.ToString());
        }
    }
}

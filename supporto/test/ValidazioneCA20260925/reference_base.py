...
import numpy as np
ROOT=Path(__file__).resolve().parents[2]/'artefatti/validazione_ca_2026_09_25'
...
def response(s,plane,law='parabola',order=6,net=True):
 ...
 return {'N':-out[0]/1000,'Mx':out[1]/1e6,'My':out[2]/1e6,'sigma_c':-float(max(cs(np.array(corners)))),'sigma_s':max((abs(v) for v in stresses), default=0),'bars':stresses,'eps_bars':strains,'plane':list(plane)}

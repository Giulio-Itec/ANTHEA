...
internal sealed partial class ConcreteWorkspace
{
    internal async Task VerifyFeatures(string directory)
    {
        ...
        Check(!shearForm!.Editors.ContainsKey("rami_x") && !shearForm.Editors.ContainsKey("rami_y"), "Rami staffe esposti una sola volta per pagina");
        Check(stirrupForms.Where(f => f.Editors.ContainsKey("rami_x")).Count(f => f.Editors["rami_x"] is System.Windows.Controls.TextBox { IsReadOnly: false }) == 1, "Unica sorgente modificabile dei rami staffa");
        Check(((System.Windows.Controls.TextBox)coverDetailingGeometry.Editors["cover_mm"]).IsReadOnly, "Copriferro nei dettagli duplicato come input");
        Check(JsonNode.DeepEquals(Result!["riepilogo_verifiche"], J.Node(CollectVerificationSummaries())), "Riepilogo esportato diverso dall'interfaccia");
        var shearReport = BuildReport("Taglio · grafico unico", ["taglio", "grafici"]);
        File.WriteAllBytes(Path.Combine(directory, "report_taglio.docx"), shearReport);
        using (var zip = new System.IO.Compression.ZipArchive(new MemoryStream(shearReport)))
        {
            Check(zip.Entries.Count(e => e.FullName.StartsWith("word/media/")) == 1, "Figura delle staffe duplicata nel report");
            using var reader = new StreamReader(zip.GetEntry("word/document.xml")!.Open());
            Check(reader.ReadToEnd().Contains("Riepilogo delle verifiche"), "Riepilogo comune assente dal report");
        }

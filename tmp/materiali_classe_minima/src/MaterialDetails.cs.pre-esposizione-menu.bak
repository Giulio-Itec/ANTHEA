...
public sealed partial class MaterialWindow
{
    ...
    UIElement BuildDetails(UIElement properties)
    {
        ...
        ntcOptions=Stack(
            ...
            AutoNumeric("ntcCmin","Cmin pertinente · fck","MPa"),
            Text("Auto: soglia Cmin della tabella C4.1.IV, aggiornata dalle esposizioni selezionate (25 / 30 / 35 MPa). Modificabile per una specifica pertinente diversa. I requisiti della miscela restano da verificare separatamente.",12),
            ntcQuality);
    ...
    void RefreshDetails()
    {
        if(!detailsReady || choice.SelectedIndex<0 || updatingDeviation || updatingAutoMix) return;
        UpdateDeviation();
        RefreshBond();
    ...
    {
        var n=NtcCover.Calculate(active,fck,p,choices["ntcElement"].SelectedIndex==1,ntcQuality.IsChecked==true,Optional("ntcCmin","Cmin pertinente",12,90));
        var c=n.Cover;

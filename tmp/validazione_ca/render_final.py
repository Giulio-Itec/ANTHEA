...
p=Path('tmp/validazione_ca/final_pages');p.mkdir(exist_ok=True)
d=pdfium.PdfDocument('tmp/validazione_ca/final.pdf')
print('PAGES',len(d))
texts=[]
...
Path('tmp/validazione_ca/final_text.txt').write_text('\n\n'.join(f'PAGE {i+1}\n{t}' for i,t in enumerate(texts)),encoding='utf-8')

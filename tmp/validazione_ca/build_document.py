...
manifest=json.loads((ROOT.parents[1]/'lib'/'Checker'/'manifest.json').read_text())
rows=[]
for name in ['GPCChecker.Concrete.dll','GPCModel.dll','GPCGeometry.dll']:
 item=next(z for z in manifest['assemblies'] if z['file']==name)
 rows.append([name,item.get('assemblyVersion',''),sha256((ROOT.parents[1]/'lib'/'Checker'/name).read_bytes()).hexdigest()[:16]])
...
p('R5. ANTHEA, sorgenti locali della revisione identificata nel capitolo precedente: X.Core/CheckerSection.cs, ConcreteMaterials.cs, ConcreteStandards.cs, Ntc2018Checks.cs, SectionShearGeometry.cs; documentazione docs/calcestruzzo-interfaccia.md; inventario delle librerie lib/Checker/manifest.json. Le informazioni sul comportamento del software sono separate dai riferimenti normativi.')

    ...
    protected override void OnStartup(StartupEventArgs e)
    {
        ...
        if (e.Args.Contains("--check"))
        {
            window.Loaded += (_, _) =>
            {
                ...
                {
                    window.Check();
                    File.WriteAllText(Path.Combine(AppContext.BaseDirectory, "verifica.txt"), "OK: proprietà, esposizioni, copriferro, acqua efficace, validazione input e tre colonne a 1500/1200 px.");
                    Shutdown(0);
                }
                catch (Exception ex)
                {
...
public sealed partial class MaterialWindow : Window
{
    ...
    {
        Title="ANTHEA · Materiali · Calcestruzzo"; Width=Math.Min(1500,SystemParameters.WorkArea.Width); Height=850; MinWidth=740; MinHeight=500;
        ...
        root.Children.Add(new ScrollViewer { Content=body, HorizontalScrollBarVisibility=ScrollBarVisibility.Auto, VerticalScrollBarVisibility=ScrollBarVisibility.Disabled });
        table.SizeChanged+=(_,_)=>FitTable();
        Content=root; choice.SelectedItem="C30/37";
    }
    void FitTable()
    {
        var available=Math.Max(290,table.ActualWidth-4);
